<?php
$h_group = get_sub_field('h6_font');

if ($h_group) {
    $preview_flex = $h_group['preview_font'] ?? null;
    $title = '';
    if ($preview_flex && isset($preview_flex[0]['preview_font_h6'])) {
        $title = $preview_flex[0]['preview_font_h6'];
    }

    $style_group = $h_group['style'] ?? [];
    $font_family = $style_group['font_family'] ?? "'Times New Roman', serif";
    $font_size   = $style_group['default_font_size'] ?: '24px';
...
<h6 class="title-preview-h6" style="<?php echo esc_attr($inline_style ?? ''); ?>">
    <?php echo esc_html($title ?? 'Preview Font H6'); ?>
</h6>
